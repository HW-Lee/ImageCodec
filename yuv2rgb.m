function rgb = yuv2rgb(path, imSize, yuvformat, convmtrx)
%Converts YUV to RGB
...
%Modified by HW-Lee:
%   To read the YUV file with specific size
data = fread(fopen(path), 'uchar');
Y = reshape( data(1:prod(imSize)), imSize )';
data(1:prod(imSize)) = []; imSize = imSize / 2;
U = reshape( data(1:prod(imSize)), imSize )';
data(1:prod(imSize)) = [];
V = reshape( data(1:prod(imSize)), imSize )';
